...
import { NewRecruitClient, NRCredentials } from './newrecruit-client.js';
import {
  scanArmyLists,
  parseMarkdownArmyList,
  toPlainTextFormat,
  toRosterFormat,
  getListSummary,
  ParsedArmyList,
} from './army-list-converter.js';
...
function getCredentials(): NRCredentials | null {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const textResponse = (text: string): { content: TextContent[] } => ({
    content: [{ type: 'text', text }],
  });

  // Helper to get NR client
  const getClient = (): NewRecruitClient => {
    const creds = getCredentials();
    if (!creds) {
      throw new Error(
        'New Recruit credentials not configured. Set NR_LOGIN and NR_PASSWORD environment variables.'
      );
    }
  ...
  try {
    switch (name) {
      ...
      case 'nr_list_tournaments': {
        const client = getClient();
        const gameSystemId = (args as { game_system_id?: number }).game_system_id;
        ...
        const formatted = tournaments
          ...
          .join('\n');
        return textResponse(
          `Recent tournaments (showing ${Math.min(20, tournaments.length)} of ${tournaments.length}):\n${formatted}`
        );
      }

      case 'nr_get_tournament': {
        const client = getClient();
        const tournamentId = (args as { tournament_id: string }).tournament_id;
      ...
      case 'nr_get_player_lists': {
        const client = getClient();
        const playerName = (args as { player_name: string }).player_name;
        ...
        const formatted = reports
          .map((r) => {
            ...
            if (r.faction) info += ` | Faction: ${r.faction}`;
            if (r.ranking) info += ` | Rank: ${r.ranking}`;
            if (r.exported_list) {
      ...
      case 'nr_get_tournament_lists': {
        ...
        const formatted = reports
          ...
          .join('\n');
        return textResponse(
          `${reports.length} lists in tournament:\n${formatted}`
        );
      }
      ...
      case 'local_get_army': {
        let filePath = (args as { file_path: string }).file_path;
      ...
      case 'local_export_for_nr': {
        let filePath = (args as { file_path: string; format?: string }).file_path;
        const format = (args as { file_path: string; format?: string }).format || 'plain';
        ...
        return textResponse(
          `Exported "${parsed.name}" in ${format} format:\n\n` +
            `---\n${exported}\n---\n\n` +
      ...
      case 'nr_import_list': {
        const client = getClient();
        const { tournament_id, player_name, output_faction, output_filename } = args as {
          tournament_id: string;
          player_name: string;
          output_faction: string;
        ...
        if (reports.length === 0) {
          return textResponse(
            `No list found for player "${player_name}" in tournament ${tournament_id}`
          );
        }
        ...
        if (!report.exported_list) {
          return textResponse(
            `Player "${player_name}" has no exported list in tournament ${tournament_id}`
          );
        }
