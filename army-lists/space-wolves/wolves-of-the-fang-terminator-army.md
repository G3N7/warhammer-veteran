# Wolves of the Fang - Space Wolves Terminator Army

**Faction:** Space Marines (Space Wolves)
**Detachment:** Saga of the Beastslayer
**Army Theme:** Maximum legal Terminators (30 models) with legendary heroes
**Playstyle:** Elite castle with Deep Strike pressure and CP generation

> **⚠️ 10TH EDITION WARGEAR UPDATE (Dec 2025):**
> The new Wolf Guard Terminators kit and datasheet **removed thunder hammers, chainfists, and power fists from regular models**. Regular Terminators now use **master-crafted power weapons (S5 AP-2 D2)** instead of S8+ melee weapons. This is weaker on paper BUT remains competitive due to:
> - **Lethal Hits detachment ability:** 6s to hit auto-wound vs Characters/Monsters/Vehicles
> - **Volume of attacks:** 7 shield models = 21 attacks = ~3.5 auto-wounds from crits alone
> - **Verified damage:** Arjac's 10-man unit deals ~20 damage vs T10 targets in one activation
> - **Local play:** Model your weapons as hammers/axes - just call them "master-crafted power weapons" for rules

---

## Table of Contents

1. [Army Lists (500/1000/2000pt)](#army-lists)
2. [Tactical Overview by Points Level](#tactical-overview-by-points-level)
3. [How to Play Each List](#how-to-play-each-list)
4. [Weapon Loadouts & Unit Compositions](#weapon-loadouts--unit-compositions)
5. [Unit Details & Datasheets](#unit-details--datasheets)
6. [Rules Reference](#rules-reference)
7. [Painting & Purchase Guide](#painting--purchase-guide)
8. [3D Printing Guide](#3d-printing-guide)

---

## Army Lists

### 500pt - "Wolf Pack"

| Unit | Models | Role | Points |
|------|--------|------|--------|
| Arjac Rockfist | 1 | Warlord (leads Terminators) | 95 |
| → Enhancement: Hunter's Instincts | - | Reroll wounds vs big targets | 20 |
| Wolf Guard Terminators | 10 | Primary Castle (Arjac attached) | 340 |
| Fenrisian Wolves | 5 | Screening / Fast Grab | 40 |
| **TOTAL** | **16 models** | | **495pts** |

**Army Composition:**
- 1 Epic Hero (Arjac)
- 10 Terminators (3W each = 30 wounds)
- 5 Wolves (1W each = 5 wounds)
- **Total: 16 models, 41 wounds**

---

### 1000pt - "Fang of Fenris"

| Unit | Models | Role | Points |
|------|--------|------|--------|
| Logan Grimnar | 1 | Warlord (leads 10-man) | 110 |
| → Enhancement: Blade of the Slayer | - | Precision for Character sniping | 25 |
| Arjac Rockfist | 1 | Leads 5-man Terminators | 95 |
| → Enhancement: Hunter's Instincts | - | Reroll wounds vs big targets | 20 |
| Wolf Guard Terminators | 10 | Primary Castle (Logan attached) | 340 |
| Wolf Guard Terminators | 5 | Objective Holders (Arjac attached) | 170 |
| Wolf Guard Terminators | 5 | Deep Strike Alpha | 170 |
| Blood Claws | 5 | Melee Screen / Counter-charge | 70 |
| **TOTAL** | **27 models** | | **1,000pts** |

**Army Composition:**
- 2 Epic Heroes (Logan + Arjac)
- 20 Terminators (3W each = 60 wounds)
- 5 Blood Claws (2W each = 10 wounds)
- **Total: 27 models, 82 wounds**

**Rule of Three Compliance:** 3x Wolf Guard Terminators units (LEGAL ✓)

---

### 2000pt - "Wolves of the Fang"

| Unit | Models | Role | Points |
|------|--------|------|--------|
| **CHARACTERS** | | | |
| Logan Grimnar | 1 | Warlord (leads 10-man) | 110 |
| → Enhancement: Blade of the Slayer | - | Precision for Character sniping | 25 |
| Arjac Rockfist | 1 | Leads 10-man Terminators | 95 |
| → Enhancement: Hunter's Instincts | - | Reroll wounds vs big targets | 20 |
| **ELITES** | | | |
| Bjorn the Fell-Handed | 1 | CP Generator (+1CP/turn) | 170 |
| Venerable Dreadnought | 1 | Charge Aura (+1 to Advance/Charge) | 140 |
| Wolf Guard Terminators | 10 | Primary Castle (Logan attached) | 340 |
| Wolf Guard Terminators | 10 | Secondary Castle (Arjac attached) | 340 |
| Wolf Guard Terminators | 10 | Backfield Objective Holders | 340 |
| **TROOPS** | | | |
| Blood Claws | 10 | Aggressive Melee Pressure | 140 |
| Grey Hunters | 10 | Mid-field Flex / OC3 Holders | 140 |
| **FAST ATTACK** | | | |
| Fenrisian Wolves | 10 | Screening / Fast Grab (12" Move) | 80 |
| **TOTAL** | **64 models** | | **1,940pts** |

**60pts unspent** (strict 2000pt cap - cannot exceed)

**Army Composition:**
- 2 Epic Heroes (Logan + Arjac = 11W)
- 2 Dreadnoughts (Bjorn + Venerable = 16W)
- 30 Terminators (3W each = 90W total) **← MAXIMUM LEGAL under Rule of Three**
- 10 Blood Claws (2W each = 20W)
- 10 Grey Hunters (2W each = 20W)
- 10 Fenrisian Wolves (1W each = 10W)
- **Total: 64 models, 167 wounds**

**Rule of Three Compliance:** 3x Wolf Guard Terminators units (LEGAL ✓)

---

## Tactical Overview by Points Level

### 500pt - "Wolf Pack"

**Strengths:**
- Extremely durable (10x Terminators with 2+/4++ saves)
- Arjac's T6 and Hunter's Instincts shred big targets
- Wolves provide fast screening and objective grabbing (12" move)

**Weaknesses:**
- Only 16 models (limited board control)
- No Deep Strike (all units deploy on board)
- No redundancy (if Terminators die, you lose)
- Limited CP (3 starting, no generation)

**Game Plan:**
1. Deploy wolves 9" forward to screen charges
2. Castle Terminators on center objective with fire lanes
3. Use Oath of Moment each turn on priority target
4. Wolves grab undefended objectives or tie up enemy shooters
5. Grind forward slowly (5" move, no Advance bonuses at this level)

---

### 1000pt - "Fang of Fenris"

**Strengths:**
- Dual legendary heroes (Logan + Arjac) with complementary enhancements
- Deep Strike 5-man Terminators for backfield pressure
- Blood Claws provide durable screening (2W, 3+ save) and counter-charge
- 20 Terminators split across 3 units = redundancy

**Weaknesses:**
- No CP generation (no Bjorn)
- No charge aura (no Venerable Dread)
- Blood Claws suffer Headstrong penalties (Ld modifiers)

**New Tactical Depth vs 500pt:**
- **Deep Strike Alpha:** 5-man drops Turn 2+ into enemy backfield or undefended objective (9" away from enemies)
- **Dual Castle:** Logan's 10-man holds center, Arjac's 5-man defends home base
- **Enhancement Synergy:**
  - Logan's Blade of Slayer (Precision) snipes enemy Characters
  - Arjac's Hunter's Instincts (reroll wounds) vs Monsters/Vehicles/Characters
- **Blood Claws Role:** Screen charges, counter-charge weak infantry, tie up enemy melee threats with Berserk Charge

**Game Plan:**
1. **Turn 1:** Deploy Logan's 10-man + Arjac's 5-man on center objectives. Blood Claws screen 9" forward or hold mid-field.
2. **Turn 2:** Deep Strike 5-man into enemy backfield (target: undefended objective, artillery, Characters). Blood Claws charge or screen.
3. **Turn 3+:** Logan's brick advances slowly. Arjac's 5-man defends home base. Deep Strike unit disrupts enemy plans.
4. **Target Priority:** Logan snipes Characters with Precision. Arjac mulches big targets with reroll wounds.
5. **CP Management:** Start with 3CP. Save for critical Stratagems (Armour of Contempt, Counter-offensive).

---

### 2000pt - "Wolves of the Fang"

**Strengths:**
- **Maximum Legal Terminators:** 30 models (Rule of Three: 3x 10-man units)
- **CP Generation:** Bjorn gives +1CP per turn (sustainable Stratagem usage)
- **Charge Aura:** Venerable Dread gives +1 to Advance/Charge within 6" (9" Deep Strike charge becomes 8")
- **Model Count:** 64 models with 167 wounds (excellent durability and board presence)
- **Flexible Troops:** Blood Claws (aggressive melee) + Grey Hunters (OC3 mid-field holders)
- **Fast Screening:** 10 Fenrisian Wolves (12" move) tie up shooters and grab flanks

**Weaknesses:**
- Elite army = vulnerable to attrition (lose trades inefficiently)
- Limited ranged firepower (mostly storm bolters + assault cannons)
- Slow movement (5" Terminators, 7" power armor)
- Expensive models (if Terminators die, hard to recover board control)

**New Tactical Depth vs 1000pt:**
- **Bjorn's CP Engine:** +1CP per turn = 8CP total by Turn 5 (can use Stratagems every phase)
- **Venerable Dread Aura:** Park near Deep Strike landing zone. 9" charge becomes 8" with +1 to charge.
- **Triple Threat:** 3x 10-man Terminator bricks = can't be focused down easily
- **Power Armor Support:** Blood Claws + Grey Hunters add model count, melee pressure, and objective control
- **Deep Strike Flexibility:** Can reserve 1-2 Terminator units for Turn 2+ alpha strike

**Game Plan (Standard Deployment):**
1. **Deployment:** Logan's 10-man + Arjac's 10-man on center objectives. Third 10-man holds backfield. Blood Claws mid-field for counter-charge. Grey Hunters on flank. Wolves screen. Dreads positioned for auras (Venerable near Terminators, Bjorn with fire lanes).
2. **Turn 1:** Castle up, activate Oath of Moment, Bjorn provides fire support. Wolves grab flanks or screen.
3. **Turn 2:** Blood Claws charge or screen. Grey Hunters hold mid-field with OC3. Terminators advance with Dread aura.
4. **Turn 3+:** Grind opponents down through superior durability. Hold 3+ objectives. Use Bjorn's CP for defensive Stratagems.
5. **Win Condition:** Preserve Terminators, trade power armor efficiently, dominate objectives late game.

**Alternative: Deep Strike Deployment**
1. **Reserve:** Place Arjac's 10-man in Deep Strike Reserve.
2. **Turn 1:** Deploy Logan's 10-man + third 10-man on center. Power armor holds flanks. Wolves screen.
3. **Turn 2:** Deep Strike Arjac's 10-man into enemy backfield near Venerable Dread for +1 to charge aura (8" charge needed instead of 9"). Use Hunter's Instincts to shred backfield targets.
4. **Turn 3+:** Arjac's squad disrupts enemy backfield, forces split attention. Logan's castle grinds forward.
5. **Win Condition:** Two-front pressure overwhelms opponent's ability to respond. Bjorn's CP keeps Stratagems flowing.

**Why This Works:**
- Bjorn makes CP sustainable (can afford 1-2 Stratagems per turn)
- Venerable Dread's +1 to charge makes Deep Strike viable (8" charge = ~41% success vs 9" = ~28%)
- Power armor adds disposable wounds (trade Blood Claws/Grey Hunters to preserve Terminators)
- Wolves tie up enemy shooters and grab flanks

---

## How to Play Each List

### 500pt - Defensive Castle

**Deployment Priority:**
1. Wolves deploy 9" forward to screen charges
2. Terminators (with Arjac) deploy on center objective with maximum fire lanes
3. Position to defend 1-2 objectives (cannot hold entire board with 16 models)

**Turn-by-Turn:**
- **Turn 1:** Activate Oath of Moment on biggest threat. Shoot storm bolters (18" range). Wolves grab undefended objective or screen.
- **Turn 2-3:** Advance Terminators slowly (5" + D6" Advance if needed). Wolves tie up enemy melee units.
- **Turn 4-5:** Hold center, weather enemy attacks with 2+/4++ saves. Use CP defensively.

**Key Stratagems:**
- Armour of Contempt (1CP): -1 AP when attacked
- Counter-offensive (1CP): Fight on death
- Only the Strongest (1CP): Reroll charges

---

### 1000pt - Dual Castle + Deep Strike

**Deployment Priority:**
1. Logan's 10-man on center objective (primary castle)
2. Arjac's 5-man on home objective (backfield defense)
3. Blood Claws mid-field or screening 9" forward
4. Deep Strike 5-man in Reserves

**Turn-by-Turn:**
- **Turn 1:** Activate Oath of Moment. Shoot priority target. Blood Claws screen or advance.
- **Turn 2:** Deep Strike 5-man into enemy backfield (9" away from all enemies). Charge with Blood Claws if target available (Berserk Charge gives extra attacks).
- **Turn 3+:** Logan's 10-man advances. Arjac's 5-man defends home. Deep Strike unit disrupts enemy plans.

**Key Stratagems:**
- Fortress of Willpower (1CP): +1 to charge rolls (use with Deep Strike unit)
- Only the Strongest (1CP): Reroll charges (combo with Fortress for reliable 9" charge)
- Armour of Contempt (1CP): Protect Terminators from AP-3 weapons

**Enhancement Usage:**
- **Logan's Blade of Slayer (Precision):** Allocate attacks to enemy Characters even if they're in units. Use against Character-led bricks.
- **Arjac's Hunter's Instincts (reroll wounds):** Reroll wound rolls vs Characters/Monsters/Vehicles. Use Arjac's 5-man to mulch big targets.

---

### 2000pt - Triple Threat Castle

**Deployment Priority (Standard):**
1. Logan's 10-man on center objective (primary castle)
2. Arjac's 10-man on secondary center objective (secondary castle)
3. Third 10-man holds backfield objective
4. Blood Claws mid-field for counter-charge
5. Grey Hunters on flank (OC3 for objective holding)
6. Wolves screen 9" forward or grab flanks (12" move)
7. Bjorn positioned with fire lanes (9" move)
8. Venerable Dread near Terminators for +1 Advance/Charge aura (6" range)

**Deployment Priority (Deep Strike Variant):**
1. Arjac's 10-man in Reserves (Deep Strike)
2. Logan's 10-man + third 10-man on center objectives
3. Power armor holds flanks and mid-field
4. Venerable Dread positions to support Deep Strike landing zone Turn 2

**Turn-by-Turn (Standard):**
- **Turn 1:** Activate Oath of Moment. Shoot priority target. Bjorn shoots with Assault cannon/Multi-melta. Wolves grab flanks.
- **Turn 2:** Advance Terminators with Venerable Dread aura (+1 to Advance/Charge). Blood Claws charge weak targets.
- **Turn 3:** Grey Hunters hold mid-field with OC3. Terminators grind forward. Use Bjorn's CP for Stratagems.
- **Turn 4-5:** Dominate objectives with model count. Preserve Terminators. Trade power armor efficiently.

**Turn-by-Turn (Deep Strike Variant):**
- **Turn 1:** Deploy standard. Wolves screen. Bjorn shoots.
- **Turn 2:** Deep Strike Arjac's 10-man into enemy backfield near Venerable Dread (if possible) for +1 to charge. Use Hunter's Instincts to shred big targets. Blood Claws charge mid-field.
- **Turn 3+:** Arjac's squad disrupts backfield. Logan's castle grinds forward. Two-front pressure overwhelms opponent.

**Key Stratagems (with Bjorn's CP):**
- **Armour of Contempt (1CP):** Use every phase if needed (Bjorn gives +1CP/turn)
- **Fortress of Willpower (1CP):** +1 to charge (combo with Venerable Dread aura for +2 total)
- **Only the Strongest (1CP):** Reroll charges (Deep Strike unit)
- **Counter-offensive (1CP):** Fight on death (protect key units)
- **Unyielding in the Face of the Foe (1CP):** Ignore modifiers to Ld (Blood Claws offset Headstrong)

**CP Math:**
- Start with 3CP
- Bjorn gives +1CP per Command phase (Turns 1-5 = +5CP total)
- Total available: 8CP across 5 turns
- Can afford 1-2 Stratagems per turn sustainably

**Enhancement Usage:**
- **Logan's Blade of Slayer (Precision):** Snipe enemy Characters in units. Focus fire to remove buffs.
- **Arjac's Hunter's Instincts (reroll wounds):** Mulch enemy Monsters/Vehicles/Characters. Use on high-value targets.

**Detachment Ability (Saga of the Beastslayer):**
- **Lethal Hits vs Characters/Monsters/Vehicles:** Crit hits (6s to hit) auto-wound. Focus fire on big targets with storm bolters + assault cannons.

---

## Weapon Loadouts & Unit Compositions

### Wolf Guard Terminators - Wargear Options

**Every model costs 34pts (wargear included).**

**Default Loadout:**
- Storm bolter (Rapid Fire 2, 18" range)
- Master-crafted power weapon (S5, AP-2, D2)

**Wargear Swaps (by model type):**

**1. Pack Leader (1 per unit - REQUIRED):**
- ✅ Can replace both weapons with: **Twin lightning claws** (S User, AP-2, D1, Twin-linked, **melee only - no ranged**)
- ✅ Can replace both weapons with: **Relic greataxe** (melee only - no ranged)

**2. Heavy Weapons (1 per 5 models):**
- ✅ Can replace storm bolter + power weapon with: **Assault cannon + Power fist** (Heavy 6, S6 AP-1 D1 Devastating Wounds + S10 AP-2 D2 melee)
- ✅ Alternative: **Cyclone missile launcher + Power fist** (Heavy 2, S9 AP-2 D3 + S10 AP-2 D2 melee)

**3. Regular Models:**
- ✅ Can replace storm bolter with: **Storm shield** (4++ invuln, lose shooting)
- ❌ **IMPORTANT:** Thunder hammers, chainfists, and power fists are NOT available to regular models in 10th Edition
- ✅ All regular models keep master-crafted power weapon (S5 AP-2 D2)

**Recommended Loadouts:**

**Shield Wall Configuration (LEGAL - Verified):**
- 1x Pack Leader: Twin lightning claws (aggressive melee, no ranged)
- 2x Heavy weapon: Assault cannon + Power fist (1 per 5 models)
- 7x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- **Pros:** Maximum durability (4++ invuln on 7/10 models, FNP 5+ from Arjac), Lethal Hits compensates for S5
- **Cons:** Limited shooting (only 2 assault cannons), S5 melee weaker vs high toughness
- **Damage vs T10:** ~20 damage per activation (Lethal Hits makes this viable!)

**Balanced Configuration:**
- 1x Pack Leader: Twin lightning claws (melee specialist)
- 2x Heavy weapon: Assault cannon + Power fist
- 5x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- 2x Regular: Storm bolter + Master-crafted power weapon (S5 AP-2 D2)
- **Pros:** Good shooting (2 assault cannons + 2 storm bolters), solid durability, balanced
- **Cons:** Less durable than full shield wall (2 models without 4++ invuln)

**Gunline Configuration:**
- 1x Pack Leader: Twin lightning claws OR keep storm bolter + master-crafted power weapon
- 2x Heavy weapon: Assault cannon + Power fist
- 7x Regular: Storm bolter + Master-crafted power weapon (S5 AP-2 D2)
- **Pros:** Maximum shooting (9 storm bolters + 2 assault cannons), volume compensates for S5
- **Cons:** No invuln saves in melee (rely on 2+ armor only), vulnerable to AP-3+ weapons

---

### Wolf Guard Terminators - Squad Compositions

#### 500pt List

**Arjac's 10-man (Primary Castle):**
- 1x Pack Leader: Twin lightning claws (melee specialist, no ranged)
- 2x Heavy: Assault cannon + Power fist
- 7x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- **Role:** Castle on center objective. Arjac provides FNP 5+. Shield wall absorbs damage.
- **Damage Output:** ~20 damage vs T10 targets (Lethal Hits compensates for S5)

---

#### 1000pt List

**Logan's 10-man (Primary Castle):**
- 1x Pack Leader: Twin lightning claws (aggressive melee)
- 2x Heavy: Assault cannon + Power fist
- 5x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- 2x Regular: Storm bolter + Master-crafted power weapon (S5 AP-2 D2)
- **Role:** Logan leads. Castle on center objective. Balanced loadout.

**Arjac's 5-man (Objective Holders):**
- 1x Pack Leader: Twin lightning claws (melee specialist)
- 1x Heavy: Assault cannon + Power fist
- 3x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- **Role:** Arjac leads. Defends home objective. FNP 5+ from Arjac makes them durable.

**Deep Strike 5-man (Alpha Strike):**
- 1x Pack Leader: Twin lightning claws (melee specialist for alpha strike)
- 1x Heavy: Assault cannon + Power fist
- 2x Regular: Storm bolter + Master-crafted power weapon (S5 AP-2 D2)
- 1x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- **Role:** Deep Strike Turn 2+. Disrupt enemy backfield. Mix of shooting and melee.

---

#### 2000pt List

**Logan's 10-man (Primary Castle):**
- 1x Pack Leader: Twin lightning claws (aggressive melee)
- 2x Heavy: Assault cannon + Power fist
- 5x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- 2x Regular: Storm bolter + Master-crafted power weapon (S5 AP-2 D2)
- **Role:** Logan leads. Primary castle on center objective. Balanced loadout.

**Arjac's 10-man (Secondary Castle or Deep Strike):**
- 1x Pack Leader: Twin lightning claws (aggressive melee)
- 2x Heavy: Assault cannon + Power fist
- 7x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- **Role:** Arjac leads. Either deploys on board OR Deep Strikes Turn 2+. FNP 5+ aura.
- **Damage Output:** ~20 damage vs T10 (verified with corrected math)

**Third 10-man (Backfield Objective Holders):**
- 1x Pack Leader: Twin lightning claws (melee specialist)
- 2x Heavy: Assault cannon + Power fist
- 5x Regular: Storm shield + Master-crafted power weapon (S5 AP-2 D2)
- 2x Regular: Storm bolter + Master-crafted power weapon (S5 AP-2 D2)
- **Role:** Holds backfield objective. Balanced loadout. Can advance if needed.

---

### Blood Claws - Unit Composition

**Stats:** M7" | T4 | Sv3+ | W2 | Ld6+ | OC2 | 14pts per model

**Unit Composition (ALWAYS includes Pack Leader):**
- 1x Blood Claw Pack Leader (REQUIRED)
- 4-9x Blood Claws
- **Unit Size:** 5-10 models

**Default Wargear:**
- Every model: Bolt pistol + Astartes chainsword

**Wargear Swaps:**
- **Pack Leader ONLY:** Can swap Astartes chainsword for Power weapon (S+1, AP-2, D1)
- **Pack Leader ONLY:** Can swap bolt pistol for Plasma pistol (optional, not recommended)

**Recommended Loadout:**
- **Pack Leader:** Power weapon + Bolt pistol (extra melee punch)
- **Regular models:** Astartes chainsword + Bolt pistol (standard aggressive loadout)

**Squad Compositions:**

**1000pt - 5-man Blood Claws:**
- 1x Pack Leader: Power weapon + Bolt pistol
- 4x Blood Claws: Astartes chainsword + Bolt pistol
- **Role:** Screen charges, counter-charge weak infantry, tie up shooters

**2000pt - 10-man Blood Claws:**
- 1x Pack Leader: Power weapon + Bolt pistol
- 9x Blood Claws: Astartes chainsword + Bolt pistol
- **Role:** Aggressive melee pressure, model count for objectives, trade efficiently into weak units

---

### Grey Hunters - Unit Composition

**Stats:** M7" | T4 | Sv3+ | W2 | Ld6+ | OC3 | 14pts per model

**Unit Composition (ALWAYS includes Pack Leader):**
- 1x Grey Hunter Pack Leader (REQUIRED)
- 4-9x Grey Hunters
- **Unit Size:** 5-10 models

**Default Wargear:**
- Every model: Boltgun + Bolt pistol + Astartes chainsword

**Wargear Swaps:**
- **Pack Leader ONLY:** Can swap Astartes chainsword for Power weapon or Power fist
- **Pack Leader ONLY:** Can swap bolt pistol for Plasma pistol (optional, not recommended)

**Recommended Loadout:**
- **Pack Leader:** Power weapon + Boltgun + Bolt pistol (keeps shooting + melee upgrade)
- **Regular models:** Boltgun + Bolt pistol + Astartes chainsword (versatile loadout)

**Squad Compositions:**

**2000pt - 10-man Grey Hunters:**
- 1x Pack Leader: Power weapon + Boltgun + Bolt pistol
- 9x Grey Hunters: Boltgun + Bolt pistol + Astartes chainsword
- **Role:** Mid-field objective holders (OC3 = superior to Blood Claws' OC2), balanced shooting and melee

---

## Unit Details & Datasheets

### Logan Grimnar

**[Wahapedia - Logan Grimnar](https://wahapedia.ru/wh40k10ed/factions/space-marines/Logan-Grimnar)**

**Stats:** M5" | T5 | Sv2+/4++ | W6 | Ld5+ | OC1 | **110pts**

**Wargear:**
- **The Axe Morkai** (Melee): S+2, AP-3, D2, Devastating Wounds
- **Storm bolter** (Ranged): Rapid Fire 2, 18" range, S4 AP0 D1

**Special Rules:**
- **Epic Hero:** Only 1 per army
- **Leader:** Can lead Wolf Guard Terminators
- **The Great Wolf (Aura):** Friendly SPACE WOLVES ADEPTUS ASTARTES units within 6" add 1 to Advance/Charge rolls and can reroll Battleshock tests
- **Lord of the Feast:** Each time Logan destroys an enemy Character or Monster, regain D3 lost wounds

**Why Take Logan:**
- Excellent melee damage (S+2 AP-3 D2 Devastating Wounds)
- Chapter Master-level buffs (+1 Advance/Charge aura, reroll Battleshock)
- Receives Blade of the Slayer enhancement (Precision = snipe Characters)
- Lore-accurate leader for Terminator armies

**Tactical Role:**
- Attach to 10-man Terminators for primary castle
- Use Precision to snipe enemy Characters in units
- Position within 6" of other units for aura buffs
- Focus fire on Characters/Monsters to trigger Lord of the Feast (heal D3 wounds)

---

### Arjac Rockfist

**[Wahapedia - Arjac Rockfist](https://wahapedia.ru/wh40k10ed/factions/space-marines/Arjac-Rockfist)**

**Stats:** M5" | T6 | Sv2+/4++ | W5 | Ld6+ | OC1 | **95pts**

**Wargear:**
- **Foehammer** (Ranged + Melee):
  - Ranged: 12" range, A1, BS2+, S10 AP-3 D4, [ONE SHOT]
  - Melee: S x2, AP-3, D4
- **Storm shield** (Wargear): 4++ invuln

**Special Rules:**
- **Epic Hero:** Only 1 per army
- **Leader:** Can lead Wolf Guard Terminators
- **Champion of the Kingsguard (Aura):** While leading, attached unit has Feel No Pain 5+
- **Anvil of Fenris:** Each time Arjac makes a melee attack, on unmodified wound roll of 6, improve AP by 3 (AP-3 becomes AP-6)

**Why Take Arjac:**
- T6 = more durable than Logan vs S6-S10 attacks
- Feel No Pain 5+ aura for attached unit = incredible durability
- S x2 melee (S12 with T6 base) hits extremely hard
- Receives Hunter's Instincts enhancement (reroll wounds vs Characters/Monsters/Vehicles)
- Foehammer ranged attack (12" S10 AP-3 D4) can delete Characters Turn 1

**Tactical Role:**
- Attach to 5-man or 10-man Terminators for FNP 5+ aura
- Use Hunter's Instincts to reroll wounds vs big targets
- Throw Foehammer Turn 1 at enemy Character (12" range, ONE SHOT)
- Position aggressively for melee (S12 AP-3 D4 mulches everything)

---

### Bjorn the Fell-Handed

**[Wahapedia - Bjorn the Fell-Handed](https://wahapedia.ru/wh40k10ed/factions/space-marines/Bjorn-The-Fell-Handed)**

**Stats:** M8" | T9 | Sv2+/5++ | W8 | Ld6+ | OC3 | **170pts**

**Wargear (Choose one main weapon):**
- **Assault cannon** (Heavy 6, 24" range, S6 AP-1 D1 Devastating Wounds) **← RECOMMENDED**
- **Helfrost cannon** (Heavy D3, 24" range, S8 AP-3 D3)
- **Multi-melta** (Heavy 2, 18" range, S9 AP-4 DD6, Melta 2)
- **Heavy flamer** (Heavy D6, 12" range, S5 AP-1 D1 Ignores Cover, Torrent) **← ALWAYS INCLUDED**
- **Trueclaw** (Melee, S12 AP-2 D3 Lethal Hits) **← ALWAYS INCLUDED**

**Special Rules:**
- **Epic Hero:** Only 1 per army
- **Ancient Tactician:** Gain +1CP at start of each Command phase (including opponent's turns)
- **Legendary Tenacity:** Each time an attack is allocated to Bjorn, if attacker's Strength is greater than Bjorn's Toughness, subtract 1 from Damage (min 1)
- **Feel No Pain 5+**
- **Deadly Demise 1**

**Why Take Bjorn:**
- **CP Generation:** +1CP per turn = sustainable Stratagem usage (8CP total across 5 turns)
- Extremely durable (T9, 2+/5++, FNP 5+, Legendary Tenacity)
- Excellent ranged damage (Assault cannon recommended for Devastating Wounds)
- Good melee damage (Trueclaw S12 AP-2 D3 Lethal Hits)
- Lore-accurate with Space Wolves Terminators

**Tactical Role:**
- Position with fire lanes (8" move, 24" assault cannon range)
- Generate +1CP per turn (use for Stratagems)
- Shoot priority targets (Assault cannon = 6 shots with Devastating Wounds)
- Melee if charged (Trueclaw S12 Lethal Hits)

**Recommended Loadout:**
- Assault cannon (anti-infantry, Devastating Wounds synergizes with Lethal Hits detachment)
- Heavy flamer (Torrent, Ignores Cover)
- Trueclaw (melee)

---

### Venerable Dreadnought

**[Wahapedia - Venerable Dreadnought](https://wahapedia.ru/wh40k10ed/factions/space-marines/Venerable-Dreadnought)**

**Stats:** M8" | T9 | Sv2+ | W8 | Ld6+ | OC3 | **140pts**

**Wargear (Recommended loadout):**
- **Fenrisian greataxe** (Melee, S14 AP-3 D6) **← RECOMMENDED**
- **Blizzard shield** (Wargear, 4++ invuln) **← RECOMMENDED**
- **Heavy flamer** (Heavy D6, 12" range, S5 AP-1 D1 Ignores Cover, Torrent) **← RECOMMENDED**

**Special Rules:**
- **Fervour of the Ancients (Aura):** While friendly ADEPTUS ASTARTES INFANTRY units are within 6", add 1 to Advance rolls and Charge rolls
- **Oath of Moment** (like all Space Marines)
- **Deadly Demise 1**

**Why Take Venerable Dreadnought:**
- **+1 to Charge Aura:** CRITICAL for Deep Strike charges (9" becomes 8" = ~41% success vs ~28%)
- **+1 to Advance Aura:** Helps Terminators advance faster (5" + D6" + 1" = average 8.5" move)
- Blizzard shield gives 4++ invuln (matches Terminators)
- Fenrisian greataxe hits like a truck (S14 AP-3 D6 = deletes Monsters/Vehicles)
- Cheaper than Bjorn (140pts vs 170pts)

**Tactical Role:**
- **CRITICAL:** Position within 6" of Terminators for +1 Advance/Charge aura
- Support Deep Strike units (land Terminators near Dread for +1 to charge)
- Advance with Terminator castle (8" move keeps pace with 5" Terminators)
- Melee if charged (S14 AP-3 D6 deletes big targets)

**Why Fenrisian Greataxe + Blizzard Shield:**
- Greataxe (S14 AP-3 D6) > other melee weapons (can wound T12-T14 on 4s-5s)
- Blizzard shield (4++ invuln) matches Terminator durability
- Heavy flamer (Torrent, Ignores Cover) clears screens

---

### Wolf Guard Terminators

**[Wahapedia - Wolf Guard Terminators](https://wahapedia.ru/wh40k10ed/factions/space-marines/Wolf-Guard-Terminators)**

**Stats:** M5" | T5 | Sv2+/4++ | W3 | Ld6+ | OC1 | **34pts per model**

**Unit Composition:**
- 1x Wolf Guard Pack Leader (REQUIRED)
- 4 or 9x Wolf Guard Terminators
- **Unit Size:** 5 or 10 models

**Wargear:** See [Weapon Loadouts & Unit Compositions](#weapon-loadouts--unit-compositions) section above

**Special Rules:**
- **Deep Strike:** Can be set up in Reserves, arrive Turn 2+ more than 9" away from enemies
- **Terminator Armour:** 2+ save, 4++ invuln
- **Can be led by:** Logan Grimnar OR Arjac Rockfist (Epic Heroes)

**Why Take Wolf Guard Terminators:**
- Elite durability (2+/4++ saves, 3W per model)
- Deep Strike flexibility (can alpha strike backfield)
- Maximum legal: 30 models (3x 10-man units under Rule of Three)
- Synergizes with Saga of the Beastslayer (Lethal Hits vs Characters/Monsters/Vehicles)

**Tactical Role:**
- Castle on objectives with 2+/4++ saves
- Deep Strike into enemy backfield Turn 2+
- Absorb damage for power armor units
- Focus fire on Characters/Monsters/Vehicles (Lethal Hits detachment ability)

---

### Blood Claws

**[Wahapedia - Blood Claws](https://wahapedia.ru/wh40k10ed/factions/space-marines/Blood-Claws)**

**Stats:** M7" | T4 | Sv3+ | W2 | Ld6+ | OC2 | **14pts per model**

**Unit Composition:** See [Weapon Loadouts & Unit Compositions](#weapon-loadouts--unit-compositions) section above

**Special Rules:**
- **Berserk Charge:** Each time this unit ends a Charge move, until end of turn, melee weapons gain [SUSTAINED HITS 1]
- **Headstrong:** Each time this unit takes a Battleshock test, subtract 1 from result

**Why Take Blood Claws:**
- Aggressive melee pressure (Berserk Charge = Sustained Hits 1 when charging)
- Cheap wounds (14pts for 2W, 3+ save)
- Model count for objectives (OC2)
- Trade efficiently into weak infantry
- Pack Leader's power weapon adds extra punch

**Tactical Role:**
- Screen charges (9" forward deployment)
- Counter-charge weak infantry (Berserk Charge triggers Sustained Hits 1)
- Tie up enemy shooters (force them into melee)
- Trade wounds to preserve Terminators

**Weakness:**
- Headstrong = subtract 1 from Battleshock tests (vulnerable to Ld modifiers)
- Use Unyielding in the Face of the Foe Stratagem (1CP) to ignore modifiers if needed

---

### Grey Hunters

**[Wahapedia - Grey Hunters](https://wahapedia.ru/wh40k10ed/factions/space-marines/Grey-Hunters)**

**Stats:** M7" | T4 | Sv3+ | W2 | Ld6+ | OC3 | **14pts per model**

**Unit Composition:** See [Weapon Loadouts & Unit Compositions](#weapon-loadouts--unit-compositions) section above

**Special Rules:**
- Versatile troops (balanced shooting and melee)
- **OC3** (superior to Blood Claws' OC2)

**Why Take Grey Hunters:**
- **OC3 = best objective holders** in power armor
- Balanced loadout (boltgun + chainsword = shooting + melee)
- Flexible mid-field holders
- No Headstrong penalties (unlike Blood Claws)
- Pack Leader's power weapon adds melee capability

**Tactical Role:**
- Hold mid-field objectives with OC3
- Shoot priority targets (boltguns)
- Counter-charge if needed (chainswords + power weapon)
- Trade efficiently into enemy troops

---

### Fenrisian Wolves

**[Wahapedia - Fenrisian Wolves](https://wahapedia.ru/wh40k10ed/factions/space-marines/Fenrisian-Wolves)**

**Stats:** M12" | T3 | Sv6+ | W1 | Ld7+ | OC0 | **8pts per model**

**Unit Size:** 5 or 10 models

**Wargear:**
- Teeth and claws (Melee, S4 AP0 D1)

**Special Rules:**
- **Fast movement:** 12" (can grab flanks, screen charges, tie up shooters)
- **Cheap:** 8pts per model (40pts for 5, 80pts for 10)

**Why Take Fenrisian Wolves:**
- Extremely fast (12" move = grab objectives, screen charges)
- Cheap wounds (8pts for screening duty)
- Tie up enemy shooters (force them into melee)
- Grab undefended objectives (flanks, backfield)

**Tactical Role:**
- Screen charges (9" forward deployment)
- Grab flanks (12" move reaches far objectives)
- Tie up enemy shooters (charge into gunlines)
- Trade 40-80pts to preserve 340pt Terminator units

**Weakness:**
- T3, 6+ save, 1W = dies easily
- OC0 = cannot contest objectives vs enemy units
- Use as sacrificial screening only

---

## Rules Reference

### Detachment: Saga of the Beastslayer

**[Wahapedia - Saga of the Beastslayer](https://wahapedia.ru/wh40k10ed/factions/space-marines/space-wolves#Saga-of-the-Beastslayer)**

**Detachment Ability: The Beastslayer's Wrath**
- Units from this detachment gain **[LETHAL HITS]** when targeting CHARACTER, MONSTER, or VEHICLE units
- **Lethal Hits:** Critical hit rolls (unmodified 6s to hit) automatically wound (skip wound roll)

**Enhancements (Max 1 per Character):**

1. **Blade of the Slayer (25pts):**
   - Bearer's melee weapons gain [PRECISION]
   - Each time bearer makes a melee attack that targets a CHARACTER, MONSTER, or VEHICLE, improve AP by 1 and add 1 to Damage
   - **Best on:** Logan Grimnar (Precision lets you snipe Characters in units, +1 AP and +1 D vs big targets)

2. **Hunter's Instincts (20pts):**
   - Each time bearer makes an attack that targets a CHARACTER, MONSTER, or VEHICLE, you can reroll the wound roll
   - **Best on:** Arjac Rockfist (reroll wounds with S12 melee + Foehammer ranged)

3. **The Pelt of Balewolf (15pts):**
   - Bearer has [STEALTH] (subtract 1 from hit rolls when targeted by ranged attacks)
   - **Best on:** Characters leading Deep Strike units (harder to shoot off board)

4. **Black Death (15pts):**
   - Bearer's melee weapons gain [DEVASTATING WOUNDS] (critical wounds inflict mortal wounds)
   - **Best on:** High-attack Characters (not optimal for Logan/Arjac who have other uses)

**Stratagems (Selected Examples):**
- **Only the Strongest (1CP):** Reroll charge rolls
- **Fortress of Willpower (1CP):** +1 to charge rolls
- **Unyielding in the Face of the Foe (1CP):** Ignore modifiers to Ld and Battleshock tests
- **Armour of Contempt (1CP):** Subtract 1 from AP of attacks targeting this unit
- **Counter-offensive (1CP):** Fight on death

---

### Space Wolves Faction Rules

**[Wahapedia - Space Wolves](https://wahapedia.ru/wh40k10ed/factions/space-marines/space-wolves)**

**Oath of Moment (Army Rule):**
- At start of your Command phase, select one enemy unit
- Until start of your next Command phase, friendly ADEPTUS ASTARTES units can reroll hit rolls when targeting that unit

**How to Use:**
- Select priority target each turn (usually biggest threat or highest value)
- Focus fire to delete unit with reroll hits
- Synergizes with Lethal Hits (more 6s to hit = more auto-wounds)

---

### Core Rules

**Rule of Three:**
- Maximum 3 units with same datasheet in matched play (except Battleline and Dedicated Transports)
- **Space Wolves Application:** Wolf Guard Terminators = max 3 units (30 models total at 10-man each)

**Points Limit:**
- HARD CAP (not "close enough")
- 500pt: Maximum 500pts
- 1000pt: Maximum 1000pts
- 2000pt: Maximum 2000pts

**Deep Strike:**
- Set up in Reserves during deployment
- Arrive Turn 2+ in Reinforcements step
- Must be more than 9" horizontally away from all enemy models
- **With Venerable Dread aura:** 9" charge becomes 8" with +1 to charge
- **With Fortress of Willpower (1CP):** Additional +1 to charge (total +2 = 7" charge needed)

**Enhancements:**
- Max 1 per Character
- ONLY Characters can take enhancements (not units)
- Must be declared during army construction

**Epic Heroes:**
- Only 1 of each named Character per army
- Logan Grimnar: Max 1
- Arjac Rockfist: Max 1
- Bjorn the Fell-Handed: Max 1

---

## Painting & Purchase Guide

### Purchase Priority (500pt → 1000pt → 2000pt)

**500pt Army:**
1. Arjac Rockfist (~$45)
2. Wolf Guard Terminators x2 boxes (~$120)
3. Fenrisian Wolves x1 box (~$35)
- **Total: ~$200**

**1000pt Army (add to 500pt):**
4. Logan Grimnar (~$50)
5. Wolf Guard Terminators x2 boxes (~$120)
6. Space Wolves Pack x1 box (~$50) - build as Blood Claws
- **Total: ~$420**

**2000pt Army (add to 1000pt):**
7. Bjorn the Fell-Handed (~$60)
8. Space Wolves Venerable Dreadnought x1 box (~$60)
9. Wolf Guard Terminators x2 boxes (~$120)
10. Space Wolves Pack x2 boxes (~$100) - build 1 as Blood Claws, 1 as Grey Hunters
11. Fenrisian Wolves x1 box (~$35)
- **Total: ~$795**

---

### Official GW Models

| Unit | Search Link | Price (USD) | Notes |
|------|-------------|-------------|-------|
| Logan Grimnar | [Search →](https://www.warhammer.com/en-US/plp?search=Logan%20Grimnar) | ~$50 | Single Character model |
| Arjac Rockfist | [Search →](https://www.warhammer.com/en-US/plp?search=Arjac%20Rockfist) | ~$45 | Single Character model |
| Bjorn the Fell-Handed | [Search →](https://www.warhammer.com/en-US/plp?search=Bjorn%20Fell-Handed) | ~$60 | Can build as Bjorn OR Venerable Dread |
| Venerable Dreadnought | [Search →](https://www.warhammer.com/en-US/plp?search=Space%20Wolves%20Venerable%20Dreadnought) | ~$60 | Same kit as Bjorn |
| Wolf Guard Terminators | [Search →](https://www.warhammer.com/en-US/plp?search=Wolf%20Guard%20Terminators) | ~$60 | 5 models per box (need 6 boxes for 30) |
| Space Wolves Pack | [Search →](https://www.warhammer.com/en-US/plp?search=Space%20Wolves%20Pack) | ~$50 | 10 models, builds Blood Claws OR Grey Hunters |
| Fenrisian Wolves | [Search →](https://www.warhammer.com/en-US/plp?search=Fenrisian%20Wolves) | ~$35 | 5 wolves per box (need 2 boxes for 10) |

---

### Painting Resources

**Official GW Painting Guides:**
- [How to Paint Space Wolves](https://www.warhammer.com/en-US/plp?search=How%20to%20Paint%20Space%20Wolves)

**Recommended Paint Scheme (Classic Space Wolves):**
- **Base:** The Fang (grey-blue armor)
- **Layer:** Russ Grey (highlight armor)
- **Wash:** Nuln Oil (shade recesses)
- **Accent:** Fenrisian Grey (extreme highlight)
- **Gold:** Retributor Armour (trim)
- **Red:** Mephiston Red (shoulder pads, seals)

**Alternative Scheme (Grimdark/Dirty):**
- Use Contrast paints: Space Wolves Grey over Grey Seer primer
- Weather with Agrax Earthshade + Typhus Corrosion
- Battle damage with Rhinox Hide + Leadbelcher

---

## 3D Printing Guide

### Model Requirements - 2000pt Army

| Quantity | Model Type | Loadout | Role |
|----------|------------|---------|------|
| **CHARACTERS & DREADNOUGHTS** | | | |
| 1 | Logan Grimnar | Axe Morkai + Storm bolter | Character |
| 1 | Arjac Rockfist | Foehammer + Storm shield | Character |
| 1 | Bjorn the Fell-Handed | Assault cannon + Heavy flamer + Trueclaw | Dreadnought |
| 1 | Venerable Dreadnought | Fenrisian greataxe + Blizzard shield + Heavy flamer | Dreadnought |
| **WOLF GUARD TERMINATORS** | | | |
| 3 | Pack Leader | Twin lightning claws | Pack Leader |
| 6 | Terminator | Assault cannon + Power fist | Heavy Weapon |
| 15 | Terminator | Storm shield + Master-crafted power weapon | Regular |
| 6 | Terminator | Storm bolter + Master-crafted power weapon | Regular |
| **BLOOD CLAWS** | | | |
| 1 | Pack Leader | Power weapon + Bolt pistol | Pack Leader |
| 9 | Blood Claw | Chainsword + Bolt pistol | Regular |
| **GREY HUNTERS** | | | |
| 1 | Pack Leader | Power weapon + Boltgun + Bolt pistol | Pack Leader |
| 9 | Grey Hunter | Boltgun + Bolt pistol + Chainsword | Regular |
| **FENRISIAN WOLVES** | | | |
| 10 | Fenrisian Wolf | Teeth and claws | Fast Attack |
| **TOTAL** | **64 models** | | |

---

*Document Generated: December 23, 2025*
*Army Builder: Tacticus (BMAD Warhammer 40k Module)*
*Rules Source: [Wahapedia](https://wahapedia.ru/wh40k10ed)*
*Points Source: Munitorum Field Manual December 2025*
*Tournament Legal: ✓ Rule of Three Compliant*
*Army Theme: Maximum legal Terminators with CP generation and charge auras*
