# Wolves of the Fang - Space Wolves Terminator Army

**Faction:** Space Marines (Space Wolves)
**Detachment:** Saga of the Beastslayer
**Army Theme:** Maximum Terminators with legendary heroes
**Playstyle:** Elite castle with Deep Strike pressure and CP generation

---

## Army Lists

### 500pt - "Wolf Pack"

| Unit | Models | Role | Points |
|------|--------|------|--------|
| Arjac Rockfist | 1 | Warlord (leads 5-man Terminators) | 105 |
| Wolf Guard Terminators | 5 | Primary Castle (Arjac attached) | 170 |
| Wolf Guard Terminators | 5 | Deep Strike / Flex | 170 |
| Fenrisian Wolves | 5 | Screening / Objective Grab | 40 |
| **TOTAL** | **16 models** | | **485pts** |

**Army Composition:**
- 1 Epic Hero (Arjac)
- 10 Terminators (3W each = 30 wounds)
- 5 Wolves (1W each = 5 wounds)
- **Total: 16 models, 40 wounds**

---

### 1000pt - "Fang of Fenris"

| Unit | Models | Role | Points |
|------|--------|------|--------|
| Logan Grimnar | 1 | Warlord (leads 10-man Terminators) | 110 |
| Arjac Rockfist | 1 | Leads 10-man Terminators | 105 |
| Wolf Guard Terminators | 10 | Primary Castle (Logan attached) | 340 |
| Wolf Guard Terminators | 10 | Secondary Castle (Arjac attached) | 340 |
| Fenrisian Wolves | 5 | Screening / Fast Grab | 40 |
| Fenrisian Wolves | 5 | Screening / Fast Grab | 40 |
| **TOTAL** | **32 models** | | **975pts** |

**Army Composition:**
- 2 Epic Heroes (Logan 8W + Arjac 6W = 14W)
- 20 Terminators (3W each = 60W)
- 10 Fenrisian Wolves (1W each = 10W)
- **Total: 32 models, 84 wounds**

**Rule of Three:** 2× Wolf Guard Terminators (legal)

---

### 2000pt - "Wolves of the Fang"

| Unit | Models | Role | Points |
|------|--------|------|--------|
| **CHARACTERS** | | | |
| Logan Grimnar | 1 | Warlord (leads 10-man Terminators) | 110 |
| Arjac Rockfist | 1 | Leads 10-man Terminators | 105 |
| Wolf Priest | 1 | Leads Blood Claws | 70 |
| *Elder's Guidance* | — | Enhancement (on Wolf Priest) | 20 |
| **ELITES** | | | |
| Bjorn the Fell-Handed | 1 | CP Generator (+1CP/turn) | 160 |
| Venerable Dreadnought | 1 | +1 Advance/Charge Aura | 130 |
| Wolf Guard Terminators | 10 | Primary Castle (Logan attached) | 340 |
| Wolf Guard Terminators | 10 | Secondary Castle (Arjac attached) | 340 |
| Wolf Guard Terminators | 10 | Backfield / Deep Strike | 340 |
| **BATTLELINE** | | | |
| Blood Claws | 10 | Melee Pressure (Wolf Priest attached) | 135 |
| Grey Hunters | 10 | Mid-field Objective Holders (OC3) | 165 |
| **FAST ATTACK** | | | |
| Fenrisian Wolves | 5 | Screening / Fast Grab | 40 |
| Fenrisian Wolves | 5 | Screening / Fast Grab | 40 |
| **TOTAL** | **65 models** | | **1,995pts** |

**Army Composition:**
- 2 Epic Heroes (Logan 8W + Arjac 6W = 14W)
- 1 Character (Wolf Priest 4W)
- 2 Dreadnoughts (Bjorn 8W + Venerable 8W = 16W)
- 30 Terminators (3W each = 90W)
- 10 Blood Claws (2W each = 20W)
- 10 Grey Hunters (2W each = 20W)
- 10 Fenrisian Wolves (1W each = 10W)
- **Total: 65 models, 174 wounds**

**Rule of Three:** 3× Wolf Guard Terminators (maximum legal)

---

## Tactical Overview

### 500pt - "Wolf Pack"

**Strengths:**
- Extremely durable core (10 Terminators with 2+/4++ saves)
- Two Terminator units = flexibility and redundancy
- Arjac's S8 melee with Anti-Monster/Vehicle 3+ shreds big targets
- Wolves provide fast screening (10" move)

**Weaknesses:**
- Only 16 models (limited board control)
- No CP generation
- No Enhancement (Arjac is Epic Hero)

**Game Plan:**
1. Deploy Arjac's 5-man on primary objective
2. Either deploy second 5-man or put in Deep Strike reserves
3. Wolves screen 9" forward or grab flanks
4. Use Oath of Moment each turn on priority target
5. Grind opponents with superior durability

---

### 1000pt - "Fang of Fenris"

**Strengths:**
- Dual legendary heroes with powerful abilities
- 20 Terminators split across 2 units
- Two wolf packs for screening coverage
- Logan's aura: +1CP cost to enemy Stratagems within 12"

**Weaknesses:**
- No CP generation (no Bjorn)
- No Enhancement (both characters are Epic Heroes)
- 25pts unspent

**Game Plan:**
1. Logan's 10-man holds center objective
2. Arjac's 10-man holds secondary or flanks
3. Both wolf packs screen and grab objectives
4. Focus fire with Oath of Moment + Lethal Hits detachment ability
5. Use Logan's Guile of the Wolf to tax enemy Stratagems

---

### 2000pt - "Wolves of the Fang"

**Strengths:**
- Maximum legal Terminators (30 models under Rule of Three)
- CP generation from Bjorn (+1CP per Command phase)
- Venerable Dreadnought aura: +1 to Advance and Charge rolls
- Wolf Priest with Elder's Guidance: +1 to wound in melee (native), +1 AP burst (once per battle)
- Full board coverage with power armor troops and wolves

**Weaknesses:**
- Elite army vulnerable to attrition
- Limited ranged firepower (mostly storm bolters)
- Slow core (6" Terminators, 7" power armor)

**Game Plan:**
1. Deploy Logan's and Arjac's Terminators on center objectives
2. Third Terminator unit holds backfield or Deep Strikes Turn 2+
3. Wolf Priest + Blood Claws push mid-field aggressively
4. Grey Hunters hold objectives with OC3
5. Wolves screen flanks and tie up enemy shooters
6. Bjorn generates CP for Stratagem usage each turn
7. Venerable Dread positions near Terminators for +1 Charge aura

**CP Math:**
- Start: 0CP (after detachment)
- Bjorn: +1CP at the start of YOUR Command phase only
- 5 battle rounds × 1 phase = +5CP over the game
- Sustainable Stratagem usage throughout

---

## Unit Profiles

### Logan Grimnar

| M | T | Sv | W | Ld | OC | Invuln | Points |
|---|---|----|----|----|----|--------|--------|
| 6" | 5 | 2+ | 8 | 6+ | 1 | 4++ | 110 |

**Weapons:**
- **Storm bolter:** 24" Rapid Fire 2, S4 AP0 D1
- **Axe Morkai – strike:** Melee, A6, S8 AP-2 D3
- **Axe Morkai – sweep:** Melee, A10, S6 AP-2 D1
- **Tyrnak and Fenrir:** Melee, A6, S5 AP-1 D1 [Extra Attacks]

**Abilities:**
- **Deep Strike, Leader**
- **High King of Fenris:** Once per battle round, one friendly Space Wolves unit in Reserves can arrive as if the battle round number is one higher
- **Guile of the Wolf (Aura):** Enemy Stratagems targeting units within 12" cost +1CP

**Leads:** Wolf Guard Terminators

---

### Arjac Rockfist

| M | T | Sv | W | Ld | OC | Invuln | Points |
|---|---|----|----|----|----|--------|--------|
| 6" | 5 | 2+ | 6 | 6+ | 1 | 4++ | 105 |

**Weapons:**
- **Foehammer (ranged):** 6" Assault, A1, S8 AP-2 D3 [Anti-Monster 3+, Anti-Vehicle 3+]
- **Foehammer (melee):** Melee, A5, S8 AP-2 D3 [Anti-Monster 3+, Anti-Vehicle 3+, Precision]

**Abilities:**
- **Deep Strike, Leader**
- **Anvil of Endurance:** While leading, when a model is destroyed by melee (before fighting), roll D6: on 4+, it can fight before being removed
- **Champion of the Kingsguard:** Reroll Hit and Wound rolls when targeting CHARACTER units

**Leads:** Wolf Guard Terminators

---

### Wolf Priest

| M | T | Sv | W | Ld | OC | Invuln | Points |
|---|---|----|----|----|----|--------|--------|
| 7" | 4 | 3+ | 4 | 6+ | 1 | 4++ | 70 |

**Weapons:**
- **Absolvor bolt pistol:** 18" Pistol, A1, S5 AP-1 D2
- **Crozius arcanum:** Melee, A5, S6 AP-1 D2

**Abilities:**
- **Leader**
- **Healing Balms:** In Command phase, return 1 destroyed model (not CHARACTER) to the unit
- **Litany of Hate:** +1 to Wound rolls for melee attacks made by the attached unit

**Leads:** Blood Claws, Grey Hunters

**Enhancement - Elder's Guidance (20pts):**
- Once per battle, bearer and attached Blood Claws unit gain +1 AP on all attacks

---

### Bjorn the Fell-Handed

| M | T | Sv | W | Ld | OC | Invuln | Points |
|---|---|----|----|----|----|--------|--------|
| 9" | 9 | 2+ | 8 | 6+ | 3 | 5++ | 160 |

**Weapons:**
- **Assault cannon:** 24" Assault, A6, S6 AP0 D1 [Devastating Wounds]
- **Heavy flamer:** 12" Torrent, AD6, S5 AP-1 D1 [Ignores Cover]
- **Trueclaw:** Melee, A6, S12 AP-2 D3 [Lethal Hits]

**Abilities:**
- **Deadly Demise 1, Feel No Pain 5+**
- **Legendary Tenacity:** -1 to Wound rolls when attack Strength > Toughness
- **Ancient Tactician:** +1CP at start of each Command phase

---

### Venerable Dreadnought

| M | T | Sv | W | Ld | OC | Invuln | Points |
|---|---|----|----|----|----|--------|--------|
| 8" | 9 | 2+ | 8 | 6+ | 3 | 4++ (with Blizzard Shield) | 130 |

**Weapons:**
- **Fenrisian greataxe – strike:** Melee, A6, S10 AP-2 D(D6+1)
- **Fenrisian greataxe – sweep:** Melee, A12, S6 AP-2 D1
- **Heavy flamer:** 12" Torrent, AD6, S5 AP-1 D1 [Ignores Cover]

**Abilities:**
- **Deadly Demise 1**
- **Fervour of the Ancients (Aura):** +1 to Advance and Charge rolls for friendly Space Wolves units within 6"
- **Blizzard Shield:** 4+ invulnerable save

**Recommended Loadout:** Fenrisian greataxe + Blizzard Shield + Heavy flamer

---

### Wolf Guard Terminators

| M | T | Sv | W | Ld | OC | Invuln | Points |
|---|---|----|----|----|----|--------|--------|
| 6" | 5 | 2+ | 3 | 6+ | 1 | 4++ | 170 (5) / 340 (10) |

**Unit Size:** 5-10 models

**Default Weapons:**
- **Storm bolter:** 24" Rapid Fire 2, S4 AP0 D1
- **Master-crafted power weapon:** Melee, A4, S5 AP-2 D2

**Wargear Options:**
- Any model can swap storm bolter for **storm shield** (bearer gains W4)
- 1 per 5 models can take **assault cannon + power fist** (replaces both weapons)
- Pack Leader can swap both weapons for **twin lightning claws** or **relic greataxe**

**Abilities:**
- **Deep Strike**
- **Rugged Resilience:** -1 to Wound rolls when attack Strength > Toughness

**Recommended Loadout (10-man):**
- 1× Pack Leader: Twin lightning claws
- 2× Heavy: Assault cannon + Power fist
- 5× Regular: Storm shield + Master-crafted power weapon
- 2× Regular: Storm bolter + Master-crafted power weapon

---

### Blood Claws

| M | T | Sv | W | Ld | OC | Points |
|---|---|----|----|----|----|--------|
| 7" | 4 | 3+ | 2 | 6+ | 2 | 135 (10) |

**Unit Size:** 10 models

**Weapons:**
- **Bolt pistol:** 12" Pistol, A1, S4 AP0 D1
- **Astartes chainsword:** Melee, A4, S4 AP-1 D1

**Abilities:**
- **Berserk Charge:** Can charge after Advancing

**Role:** Aggressive melee pressure. Wolf Priest's +1 to wound makes them devastating on the charge.

---

### Grey Hunters

| M | T | Sv | W | Ld | OC | Points |
|---|---|----|----|----|----|--------|
| 7" | 4 | 3+ | 2 | 6+ | 3 | 165 (10) |

**Unit Size:** 10 models

**Weapons:**
- **Bolt carbine:** 24" Rapid Fire 1, A2, S4 AP0 D1
- **Bolt pistol:** 12" Pistol, A1, S4 AP0 D1
- **Astartes chainsword:** Melee, A4, S4 AP-1 D1

**Abilities:**
- **Cunning Hunters:** Reroll Wound rolls of 1 (full reroll if target is on an objective)

**Role:** Mid-field objective holders. OC3 is the best in power armor.

---

### Fenrisian Wolves

| M | T | Sv | W | Ld | OC | Points |
|---|---|----|----|----|----|--------|
| 10" | 4 | 6+ | 1 | 8+ | 0 | 40 (5) |

**Unit Size:** 5 models

**Weapons:**
- **Teeth and claws:** Melee, A3, S4 AP0 D1

**Abilities:**
- **Predatory Instinct:** When enemy ends move within 9", this unit can move D6"
- **Hunting Hounds:** OC1 while within 6" of a friendly Space Wolves CHARACTER

**Role:** Screening and objective grabbing. Fast (10" move) and cheap.

---

## Detachment: Saga of the Beastslayer

**Detachment Ability - The Beastslayer's Wrath:**
- Units gain [Lethal Hits] when targeting CHARACTER, MONSTER, or VEHICLE units
- Critical hits (unmodified 6s) automatically wound

**Enhancements:**

| Enhancement | Cost | Effect |
|-------------|------|--------|
| Elder's Guidance | 20pts | Once per battle, bearer (+Blood Claws unit) gains +1 AP |
| Helm of the Beastslayer | 15pts | Reduces AP of incoming attacks by 1 |
| Hunter's Guile | 20pts | Redeploy up to 3 Thunderwolf/Wulfen/Blood Claws units |
| Wolf-Touched | 15pts | +2" Move, bearer can join Wulfen units |

**Key Stratagems:**
- **Armour of Contempt (1CP):** -1 AP when attacked
- **Only the Strongest (1CP):** Reroll charge rolls
- **Counter-offensive (2CP):** Fight on death
- **Heroic Intervention (2CP):** Intervene into enemy charge

---

## Quick Reference

### Deployment Priorities

**500pt:**
1. Arjac's 5-man on center objective
2. Second 5-man deploys or reserves for Deep Strike
3. Wolves screen forward

**1000pt:**
1. Logan's 10-man on center objective
2. Arjac's 10-man on secondary objective
3. Both wolf packs screen flanks

**2000pt:**
1. Logan's 10-man + Arjac's 10-man on center objectives
2. Third Terminator unit holds backfield or reserves
3. Wolf Priest + Blood Claws mid-field
4. Grey Hunters on flank objective
5. Wolves screen forward
6. Bjorn with fire lanes, Venerable Dread near Terminators

### Target Priority (Oath of Moment)

1. Enemy Characters (trigger Lethal Hits + Arjac bonuses)
2. Monsters/Vehicles (trigger Lethal Hits)
3. Biggest threat to your Terminators
4. Objective holders you need to remove

### Stratagem Priority

With Bjorn generating +1CP per phase:
1. **Armour of Contempt** - use liberally on Terminators
2. **Only the Strongest** - for critical charges
3. **Counter-offensive** - protect key units

---

## Model Assembly Guide

### 500pt (16 models)

| Model | Qty | Loadout |
|-------|-----|---------|
| Arjac Rockfist | 1 | Foehammer |
| Terminator Pack Leader | 2 | Twin lightning claws |
| Terminator Heavy | 2 | Assault cannon + power fist |
| Terminator Shield | 4 | Storm shield + power weapon |
| Terminator Bolter | 2 | Storm bolter + power weapon |
| Fenrisian Wolf | 5 | — |

### 1000pt (32 models)

| Model | Qty | Loadout |
|-------|-----|---------|
| Logan Grimnar | 1 | Axe Morkai + storm bolter |
| Arjac Rockfist | 1 | Foehammer |
| Terminator Pack Leader | 2 | Twin lightning claws |
| Terminator Heavy | 4 | Assault cannon + power fist |
| Terminator Shield | 10 | Storm shield + power weapon |
| Terminator Bolter | 4 | Storm bolter + power weapon |
| Fenrisian Wolf | 10 | — |

### 2000pt (65 models)

| Model | Qty | Loadout |
|-------|-----|---------|
| Logan Grimnar | 1 | Axe Morkai + storm bolter |
| Arjac Rockfist | 1 | Foehammer |
| Wolf Priest | 1 | Crozius + absolvor bolt pistol |
| Bjorn the Fell-Handed | 1 | Assault cannon + Trueclaw |
| Venerable Dreadnought | 1 | Greataxe + blizzard shield + heavy flamer |
| Terminator Pack Leader | 3 | Twin lightning claws |
| Terminator Heavy | 6 | Assault cannon + power fist |
| Terminator Shield | 15 | Storm shield + power weapon |
| Terminator Bolter | 6 | Storm bolter + power weapon |
| Blood Claw Pack Leader | 1 | Bolt pistol + chainsword |
| Blood Claw | 9 | Bolt pistol + chainsword |
| Grey Hunter Pack Leader | 1 | Bolt carbine + chainsword |
| Grey Hunter | 9 | Bolt carbine + chainsword |
| Fenrisian Wolf | 10 | — |

---

## Shopping List & Cost Summary

### 2000pt Shopping List

| Item | Quantity | Cost Each | Total |
|------|----------|-----------|-------|
| Logan Grimnar | 1 | $42 | $42 |
| Arjac Rockfist | 1 | $35 | $35 |
| Wolf Priest (Primaris Chaplain) | 1 | $38 | $38 |
| Bjorn the Fell-Handed | 1 | $55 | $55 |
| Venerable Dreadnought | 1 | $55 | $55 |
| Wolf Guard Terminators | 6 boxes | $60 | $360 |
| Blood Claws | 1 | $60 | $60 |
| Grey Hunters | 1 | $60 | $60 |
| Fenrisian Wolves | 2 | $35 | $70 |

**TOTAL COST: ~$775 USD** (for 2000pt army)

### Model Count Summary

| Points Level | Terminators | Infantry | Wolves | Characters | Dreadnoughts | Total |
|--------------|-------------|----------|--------|------------|--------------|-------|
| 500pt | 10 | 0 | 5 | 1 | 0 | **16** |
| 1000pt | 20 | 0 | 10 | 2 | 0 | **32** |
| 2000pt | 30 | 20 | 10 | 3 | 2 | **65** |

### Budget-Friendly Build Path

1. Buy Combat Patrol: Space Marines ($160) - contains Terminators + characters (proxy)
2. Add Logan Grimnar ($42) + Arjac Rockfist ($35)
3. Add Bjorn the Fell-Handed ($55)
4. Add 4x Wolf Guard Terminators boxes ($240) - need 30 total
5. Add Blood Claws + Grey Hunters ($120)
6. Add Fenrisian Wolves x2 ($70)
7. Add Venerable Dreadnought ($55) + Wolf Priest ($38)

**Approximate Total: ~$775 USD**

### Magnetization Recommendations

**HIGH PRIORITY:**
- **Terminator arms:** Magnetize to swap storm bolter ↔ storm shield ↔ assault cannon
- **Pack Leader weapons:** Twin lightning claws ↔ thunder hammer + storm shield

**MEDIUM PRIORITY:**
- **Venerable Dreadnought:** Greataxe ↔ other melee options
- **Bjorn:** Assault cannon ↔ heavy flamer ↔ multi-melta

---

<!-- BRUSHMASTER SECTION - Tacticus ignore -->
## Paint Scheme: Space Wolves

**Chapter:** Space Wolves (VI Legion)

### Palette

| Type | Role | Paint | Hex | Swatch |
|:----:|------|-------|-----|--------|
| ■ | Primary (Armour) | Wolf Grey | #6D8BA3 | <span style="background:#6D8BA3;padding:2px 12px;">&nbsp;</span> |
| ● | Shade | Blue Tone | #1E3A5F | <span style="background:#1E3A5F;padding:2px 12px;">&nbsp;</span> |
| ▲ | Highlight | Ice Storm | #B0C4DE | <span style="background:#B0C4DE;padding:2px 12px;">&nbsp;</span> |
| ▲ | Edge Highlight | Matt White | #FFFFFF | <span style="background:#FFFFFF;padding:2px 12px;">&nbsp;</span> |
| ◆ | Gold Trim | Greedy Gold | #FFD700 | <span style="background:#FFD700;padding:2px 12px;">&nbsp;</span> |
| ● | Gold Shade | Strong Tone | #5C4033 | <span style="background:#5C4033;padding:2px 12px;">&nbsp;</span> |
| ◆ | Silver Metal | Plate Mail Metal | #C0C0C0 | <span style="background:#C0C0C0;padding:2px 12px;">&nbsp;</span> |
| ● | Metal Wash | Dark Tone | #3D2B1F | <span style="background:#3D2B1F;padding:2px 12px;">&nbsp;</span> |
| ■ | Leather/Pelts | Leather Brown | #8B4513 | <span style="background:#8B4513;padding:2px 12px;">&nbsp;</span> |
| ■ | Bone/Fangs | Skeleton Bone | #E8DCC4 | <span style="background:#E8DCC4;padding:2px 12px;">&nbsp;</span> |
| ■ | Eyes/Lenses | Pure Red | #C41E3A | <span style="background:#C41E3A;padding:2px 12px;">&nbsp;</span> |
| ■ | Wolf Fur (Grey) | Ash Grey | #B2BEB5 | <span style="background:#B2BEB5;padding:2px 12px;">&nbsp;</span> |

**Shape Key:** ■ Base | ▲ Layer | ◆ Metallic | ● Wash | ★ Effect

### Quick Recipes

**Terminator Armour:** ■ Wolf Grey → ● Blue Tone → ▲ Ice Storm layer → ▲ Matt White edge
**Gold Trim:** ◆ Greedy Gold → ● Strong Tone → ◆ Greedy Gold recess avoid
**Pelts/Leather:** ■ Leather Brown → ● Dark Tone → ▲ Monster Brown drybrush
**Wolf Skulls/Fangs:** ■ Skeleton Bone → ● Soft Tone → ▲ Matt White tips
**Fenrisian Wolves:** ■ Ash Grey → ● Blue Tone → ▲ Matt White drybrush

### Space Wolves Signature Elements
- Wolf pelts and trophies on every model
- Runes and Fenrisian glyphs (freehand or transfers)
- Red eye lenses
- Bone/fang decorations
- Weathered, battle-worn look

*View color swatches: Open file in VSCode → Ctrl+Shift+V*
<!-- END BRUSHMASTER SECTION -->

---

*Document Generated: December 27, 2025*
*Document Updated: December 30, 2025*
*Points Source: Munitorum Field Manual December 2025*
*Rules Source: Space Wolves Codex Supplement (May 2025)*
*Brushmaster Agent - Paint Scheme*
