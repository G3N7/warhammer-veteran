# Competitive Wolves - Stormlance Task Force

> **Detachment:** Stormlance Task Force
> **Faction:** Space Wolves (Adeptus Astartes)
> **Philosophy:** Meta-optimized hybrid army. Dual Wolf Priests buff infantry blobs, Terminators Deep Strike, Thunderwolf Cavalry flanks, ranged fire support holds the castle.

---

## Army Lore

The Space Wolves are the VI Legion, sons of Leman Russ, the Wolf King of Fenris. Known as the Emperor's executioners, they fight with savage ferocity tempered by fierce loyalty. The Stormlance detachment represents their thundering cavalry charges -- Thunderwolf riders and swift Outriders crashing into enemy lines like a Fenrisian blizzard. This list embodies the Vlka Fenryka's preferred way of war: hit fast, hit hard, leave nothing standing.

---

## Army Identity

**Core Strengths:**
- **Advance+Charge army-wide** - Stormlance gives all ADEPTUS ASTARTES units Advance and Charge
- **Deep Strike Threat** - 2 Terminator bricks with Logan/Arjac
- **CP Generation** - Bjorn generates 1 CP per battle round on 4+
- **Dual Wolf Priest Buffs** - Two units get +1 to wound melee + model resurrection
- **Meta Coverage** - Hits 7 of top 10 competitive Space Wolves units
- **Infiltrate Pressure** - Wolf Scouts deploy forward for early board control
- **Fast Flanking** - Thunderwolf Cavalry for mobility and threat projection

**Playstyle:**
1. Deploy fire support castle (Bjorn, Gladiator Reaper, Ballistus)
2. Wolf Scouts Infiltrate forward, threaten flanks
3. Grey Hunters hold primary objectives (OC3)
4. Deep Strike Logan + Terminators T2 into enemy lines
5. Advance+Charge Blood Claws with Wolf Priest buffs
6. Thunderwolf Cavalry flanks and hunts priority targets
7. Headtakers with Wolf Priest assassinate enemy characters

---

## 2000pt List (2,000pts)

### Characters (610pts)

| Unit | Points | Role |
|------|--------|------|
| Logan Grimnar | 110 | Warlord, leads Terminators |
| Arjac Rockfist | 105 | Terminator leader, melee hammer |
| Bjorn the Fell-Handed | 160 | CP generation (4+ each round), fire support |
| Njal Stormcaller | 85 | Grey Hunters leader, PSYKER, mortal wounds |
| Wolf Priest + Feinting Withdrawal | 80 | Blood Claws leader, +1 to wound melee, Fall Back+Charge |
| Wolf Priest | 70 | Headtakers leader, +1 to wound melee |

> **Note:** Logan (foot, 110pts) leads Wolf Guard Terminators. Stormrider version (180pts) can only lead Thunderwolf Cavalry.

### Elites (450pts)

| Unit | Points | Role |
|------|--------|------|
| Wolf Guard Terminators ×5 | 170 | Brick 1 (Logan attached) - Deep Strike |
| Wolf Guard Terminators ×5 | 170 | Brick 2 (Arjac attached) - Deep Strike |
| Wolf Guard Headtakers | 110 | Character hunters (Wolf Priest attached) |

> **Headtakers:** 3 models + 3 Hunting Wolves. Devastating Wounds + Precision vs selected quarry.

### Battleline (435pts)

| Unit | Points | Role |
|------|--------|------|
| Blood Claws ×10 | 135 | Melee blob (Wolf Priest attached) |
| Blood Claws ×10 | 135 | Melee blob |
| Grey Hunters ×10 | 165 | Objective holders (Njal attached, OC3) |

### Fast Attack (115pts)

| Unit | Points | Role |
|------|--------|------|
| Thunderwolf Cavalry ×3 | 115 | Fast flanking, melee threat |

### Fire Support (310pts)

| Unit | Points | Role |
|------|--------|------|
| Gladiator Reaper | 160 | Anti-horde (volume D1 shots) |
| Ballistus Dreadnought | 150 | Anti-heavy (D6+1, D3 damage) |

> **Note:** Gladiator Reaper and Ballistus are generic Space Marine units.

### Screening (80pts)

| Unit | Points | Role |
|------|--------|------|
| Wolf Scouts ×5 | 80 | Infiltrate, early board presence |

### Summary

| Category | Points | Models |
|----------|--------|--------|
| Characters | 610 | 6 |
| Elites | 450 | 16 |
| Battleline | 435 | 30 |
| Fast Attack | 115 | 3 |
| Fire Support | 310 | 2 |
| Screening | 80 | 5 |
| **TOTAL** | **2,000** | **62** |

### Points Verification

- Logan Grimnar: 110pts
- Arjac Rockfist: 105pts
- Bjorn the Fell-Handed: 160pts
- Njal Stormcaller: 85pts
- Wolf Priest + Feinting Withdrawal: 70 + 10 = 80pts
- Wolf Priest: 70pts
- Wolf Guard Terminators ×5: 170pts × 2 = 340pts
- Wolf Guard Headtakers: 110pts
- Blood Claws ×10: 135pts × 2 = 270pts
- Grey Hunters ×10: 165pts
- Thunderwolf Cavalry ×3: 115pts (stepped pricing)
- Gladiator Reaper: 160pts
- Ballistus Dreadnought: 150pts
- Wolf Scouts ×5: 80pts (16pts × 5)

**TOTAL: 110 + 105 + 160 + 85 + 80 + 70 + 340 + 110 + 270 + 165 + 115 + 160 + 150 + 80 = 2,000pts** ✅

### Meta Coverage

| Meta Rank | Unit | Status |
|-----------|------|--------|
| #1 | Thunderwolf Cavalry | ✅ |
| #2 | Wolf Guard Terminators | ✅ |
| #3 | Logan Grimnar | ✅ |
| #5 | Arjac Rockfist | ✅ |
| #7 | Bjorn the Fell-Handed | ✅ |
| #8 | Wolf Guard Headtakers | ✅ |
| #9 | Scout Squad (Wolf Scouts) | ✅ |

---

## 1000pt List (1,000pts)

Scaled version maintaining core identity - no melee dreads per preference.

| Unit | Points | Notes |
|------|--------|-------|
| **CHARACTERS** | | |
| Logan Grimnar | 110 | Warlord, leads Terminators |
| Wolf Priest | 70 | Leads Blood Claws |
| **ELITES** | | |
| Bjorn the Fell-Handed | 160 | CP gen + fire support |
| Wolf Guard Terminators ×5 | 170 | Deep Strike with Logan |
| **BATTLELINE** | | |
| Blood Claws ×10 | 135 | Melee punch (Wolf Priest attached) |
| Grey Hunters ×10 | 165 | Objective holders (OC3) |
| **FIRE SUPPORT** | | |
| Ballistus Dreadnought | 150 | Anti-heavy ranged |
| **SCREENING** | | |
| Fenrisian Wolves ×5 | 40 | Screen |
| **TOTAL** | **1,000** | 27 models |

**Points Verification:**
- 110 + 70 + 160 + 170 + 135 + 165 + 150 + 40 = **1,000pts** ✅

---

## 500pt List - Colosseum Format (500pts)

**King of the Colosseum** compliant build.

### Colosseum Format Rules

| Rule | Requirement | This List |
|------|-------------|-----------|
| Points | 500pts max | 495pts ✅ |
| Characters | 1+ required | Wolf Priest ✅ |
| Epic Heroes | BANNED | None ✅ |
| Max Toughness | T9 | T5 (highest) ✅ |
| Infantry (non-char) | 2+ units | 3 units ✅ |

### Army List

| Unit | Points | Notes |
|------|--------|-------|
| **CHARACTERS** | | |
| Wolf Priest + Portents of Wisdom | 85 | Warlord, leads Grey Hunters, re-roll Advance |
| **INFANTRY** | | |
| Wolf Guard Terminators ×5 | 170 | Deep Strike hammer |
| Grey Hunters ×10 | 165 | Primary objective holders (OC3 = 30 OC) |
| Assault Intercessors ×5 | 75 | Trade piece, aggressive flank |
| **TOTAL** | **495** | 21 models |

### Points Verification

- Wolf Priest: 70pts
- Portents of Wisdom: 15pts
- Wolf Guard Terminators ×5: 170pts
- Grey Hunters ×10: 165pts
- Assault Intercessors ×5: 75pts (5 × 15pts)
- **Total: 70 + 15 + 170 + 165 + 75 = 495pts** ✅

### Why This Works for Colosseum

1. **3 INFANTRY units** - Grey Hunters + Terminators + Assault Intercessors (exceeds 2+ requirement)
2. **No Epic Heroes** - Wolf Priest is generic CHARACTER
3. **T5 max** - Terminators are highest toughness (well under T9 cap)
4. **Stormlance synergy** - All units benefit from Advance+Charge
5. **Portents of Wisdom** - Re-roll Advance rolls helps position for Stormlance charges

### Colosseum Tactical Notes

**Deployment:**
- Wolf Priest + Grey Hunters: Center objective (30 OC block)
- Terminators: Deploy or Deep Strike reserve based on opponent
- Assault Intercessors: Aggressive flank or secondary objective

**Game Plan:**
1. Grey Hunters castle on primary with Wolf Priest buffs (+1 to wound melee, resurrect 1 model/turn). Re-roll Advances for positioning.
2. Advance+Charge Assault Intercessors into enemy flanks
3. Deep Strike Terminators T2 to eliminate priority targets
4. Use Stormlance stratagems (Swift Hunters, Ride Them Down) for mobility advantage

---

## Assembly Priority Guide

Build order optimized for playability at each point level.

### Phase 1: 500pt Colosseum Core (Buy First)

| Kit | Units Built | Priority |
|-----|-------------|----------|
| Space Wolves Pack | Grey Hunters ×10 | 1 |
| Wolf Guard Terminators | Terminators ×5 | 2 |
| Assault Intercessors | Assault Intercessors ×5 | 3 |
| Primaris Chaplain | Wolf Priest #1 | 4 |
| Space Wolves Upgrade Pack | Wolf Priest bits | 4 |

### Phase 2: 1000pt Expansion

| Kit | Units Built | Priority |
|-----|-------------|----------|
| Logan Grimnar | Logan (foot version) | 5 |
| Bjorn the Fell-Handed | Bjorn | 6 |
| Space Wolves Pack | Blood Claws ×10 | 7 |
| Ballistus Dreadnought | Ballistus | 8 |
| Fenrisian Wolves | Wolves ×5 (for 1000pt list) | 9 |

### Phase 3: 2000pt Full Army

| Kit | Units Built | Priority |
|-----|-------------|----------|
| Wolf Guard Terminators | Terminators ×5 (second brick) | 10 |
| Arjac Rockfist | Arjac | 11 |
| Primaris Chaplain | Wolf Priest #2 | 12 |
| Wolf Guard Headtakers | Headtakers + Hunting Wolves | 13 |
| Thunderwolf Cavalry | TWC ×3 | 14 |
| Wolf Scouts | Scouts ×5 | 15 |
| Gladiator Reaper | Reaper | 16 |
| Njal Stormcaller | Njal | 17 |

---

## Shopping List

### Complete 2000pt Army

| Product | Qty | Approx USD | Notes |
|---------|-----|------------|-------|
| Logan Grimnar | 1 | $42 | Epic Hero (foot) |
| Arjac Rockfist | 1 | $35 | Epic Hero |
| Bjorn the Fell-Handed | 1 | $55 | Epic Hero |
| Wolf Guard Terminators | 2 | $120 | 5-man boxes |
| Wolf Guard Headtakers | 1 | $55 | 3 models + 3 wolves |
| Space Wolves Pack | 2 | $110 | 1× Grey Hunters, 1× Blood Claws |
| Thunderwolf Cavalry | 1 | $70 | 3-model box |
| Wolf Scouts | 1 | $35 | 5-man box |
| Assault Intercessors | 1 | $55 | 5-man box (Colosseum) |
| Gladiator Reaper | 1 | $70 | Tank kit |
| Ballistus Dreadnought | 1 | $70 | Dreadnought |
| Njal Stormcaller | 1 | $42 | Epic Hero PSYKER |
| Primaris Chaplain | 2 | $70 | Wolf Priest ×2 |
| Space Wolves Upgrade Pack | 1 | $15 | Bits for Wolf Priests |
| **TOTAL** | | **~$844** | |

---

## Detachment Rules Reference

### Stormlance Task Force

**Detachment Rule - Ride Hard, Ride Fast:**
> Each time an ADEPTUS ASTARTES unit from your army Advances, until the end of the turn, that unit is eligible to declare a charge.

**Enhancements:**
| Enhancement | Points | Restriction | Effect |
|-------------|--------|-------------|--------|
| Fury of the Storm | 25 | MOUNTED | +1 to wound on charge |
| Hunter's Instincts | 10 | MOUNTED | +1 to hit on charge |
| Portents of Wisdom | 15 | - | Re-roll Advance rolls for led unit |
| Feinting Withdrawal | 10 | - | Fall Back and Charge |

**Stratagems (Key):**
- **Swift Hunters (1CP):** After enemy Falls Back, one of your units can make a Normal Move
- **Vicious Pursuit (1CP):** +1 to wound when charging a unit that Fell Back
- **Ride Them Down (1CP):** After destroying unit in Fight phase, consolidate up to 6"

---

## Leader Attachments

| Leader | Attached To | Benefit |
|--------|-------------|---------|
| Logan Grimnar | Wolf Guard Terminators (Brick 1) | Deep Strike, +1 Attack aura |
| Arjac Rockfist | Wolf Guard Terminators (Brick 2) | Champion of the Kingsguard (+1 to wound) |
| Njal Stormcaller | Grey Hunters | PSYKER, Tempest Wrath (mortal wounds), +1 to wound melee |
| Wolf Priest #1 + Feinting Withdrawal | Blood Claws | +1 to wound melee, resurrect 1 model/turn, Fall Back+Charge |
| Wolf Priest #2 | Wolf Guard Headtakers | +1 to wound melee, resurrect 1 model/turn |

> **Verified against cache:**
> - Logan.leads = ["Wolf Guard Terminators"] ✅
> - Arjac.leads = ["Wolf Guard Terminators"] ✅
> - Njal.leads = ["Wolf Guard Headtakers", "Grey Hunters", "Blood Claws"] ✅
> - Wolf Priest.leads = ["Wolf Guard Headtakers", "Grey Hunters", "Blood Claws"] ✅

---

## Tactical Notes

### Matchup Adjustments

**vs. Horde Armies:**
- Castle Gladiator Reaper + Bjorn for volume fire
- Use Fenrisian Wolves to tie up chaff
- Wolf Scouts grab distant objectives early
- Blood Claws with Wolf Priest delete infantry blobs

**vs. -1 Damage Armies (Orks w/ Painboy, etc.):**
- Lead with Ballistus (D6+1 damage bypasses -1)
- Thunderwolf Cavalry for mobile threats
- Avoid committing Terminators to tarpits

**vs. Elite Armies:**
- Deep Strike Logan + Terminators into their backline T2
- Headtakers hunt enemy characters (Devastating Wounds + Precision)
- Grey Hunters hold home objectives (OC3), force them to come to you
- Trade Blood Claws efficiently

**vs. Vehicle-Heavy:**
- Focus Ballistus on priority armor
- Arjac has Anti-Monster/Vehicle 3+ for big targets
- Thunderwolf Cavalry can flank and threaten rear armor

### Key Combos

1. **Logan Alpha:** Deep Strike Terminators T2, charge 9"
2. **Dual Wolf Priest Buffs:** Two units get +1 to wound melee + model resurrection each turn
3. **Bjorn CP Farm:** Average 2.5 CP over 5 rounds, fuels stratagems
4. **Headtaker Assassination:** Pick quarry → Devastating Wounds + Precision = dead characters
5. **TWC Flanking:** 10" move + Advance + Charge = massive threat range
6. **Scout Forward Deploy:** Wolf Scouts Infiltrate for early board control

---

## Version History

| Date | Change |
|------|--------|
| 2026-01 | Initial creation - Hybrid Stormlance build |
| 2026-01 | Validated at 2,000pts exactly |
| 2026-01 | 500pt updated to Colosseum-compliant (Grey Hunters + Assault Intercessors) |
| 2026-01 | Major revision: Added TWC, Headtakers, 2nd Wolf Priest, Wolf Scouts. Removed melee dreads. Meta-optimized (7/10 top units). |
| 2026-01 | Added Njal Stormcaller (leads Grey Hunters), Feinting Withdrawal on Wolf Priest. Removed Fenrisian Wolves. Now exactly 2,000pts. |

---

<!-- BRUSHMASTER SECTION - Tacticus ignore -->
## Paint Scheme: Space Wolves (Great Fang)

**Chapter:** Space Wolves (VI Legion) - Great Company

### Palette

| Type | Role | Paint | Hex | Swatch |
|:----:|------|-------|-----|--------|
| ■ | Primary (Armour) | The Fang / Space Wolves Grey | #6B7B8D | <span style="background:#6B7B8D;padding:2px 12px;">&nbsp;</span> |
| ● | Shade | Dark Tone | #3D2B1F | <span style="background:#3D2B1F;padding:2px 12px;">&nbsp;</span> |
| ▲ | Highlight | Fenrisian Grey | #A8B4BE | <span style="background:#A8B4BE;padding:2px 12px;">&nbsp;</span> |
| ▲ | Edge Highlight | Matt White | #FFFFFF | <span style="background:#FFFFFF;padding:2px 12px;">&nbsp;</span> |
| ◆ | Gold Trim | Greedy Gold | #FFD700 | <span style="background:#FFD700;padding:2px 12px;">&nbsp;</span> |
| ● | Gold Shade | Strong Tone | #5C4033 | <span style="background:#5C4033;padding:2px 12px;">&nbsp;</span> |
| ◆ | Silver Metal | Plate Mail Metal | #C0C0C0 | <span style="background:#C0C0C0;padding:2px 12px;">&nbsp;</span> |
| ● | Metal Wash | Dark Tone | #3D2B1F | <span style="background:#3D2B1F;padding:2px 12px;">&nbsp;</span> |
| ■ | Skin | Tanned Flesh | #D4A574 | <span style="background:#D4A574;padding:2px 12px;">&nbsp;</span> |
| ● | Skin Shade | Flesh Wash | #C4956A | <span style="background:#C4956A;padding:2px 12px;">&nbsp;</span> |
| ■ | Pack Markings | Pure Red | #C41E3A | <span style="background:#C41E3A;padding:2px 12px;">&nbsp;</span> |
| ■ | Shoulder Trim (Great Company) | Daemonic Yellow | #F5D033 | <span style="background:#F5D033;padding:2px 12px;">&nbsp;</span> |
| ■ | Leather/Pelts | Leather Brown | #8B4513 | <span style="background:#8B4513;padding:2px 12px;">&nbsp;</span> |
| ■ | Bone/Fangs | Skeleton Bone | #E8DCC4 | <span style="background:#E8DCC4;padding:2px 12px;">&nbsp;</span> |
| ■ | Eyes/Lenses | Pure Red | #C41E3A | <span style="background:#C41E3A;padding:2px 12px;">&nbsp;</span> |

**Shape Key:** ■ Base | ▲ Layer | ◆ Metallic | ● Wash | ★ Effect

### Quick Recipes

**Armour:** ■ The Fang → ● Dark Tone recess → ▲ Fenrisian Grey layer → ▲ Matt White edge
**Gold Trim:** ◆ Greedy Gold → ● Strong Tone → ◆ Greedy Gold recess avoid
**Pack Markings:** ■ Pure Red → ● Dark Tone → ▲ Mars Red highlight
**Shoulder Trim:** ■ Daemonic Yellow → ● Soft Tone → ▲ Daemonic Yellow cleanup
**Skin:** ■ Tanned Flesh → ● Flesh Wash → ▲ Fair Skin highlight
**Pelts/Leather:** ■ Leather Brown → ● Dark Tone → ▲ Monster Brown drybrush
**Fangs/Bone:** ■ Skeleton Bone → ● Soft Tone → ▲ Matt White tips

### Space Wolves Signature Elements
- Grey-blue armour with weathered, battle-worn look
- Gold trim and Fenrisian knotwork on pauldrons
- Red pack markings and Great Company identifiers
- Yellow shoulder trim denoting Great Company
- Wolf pelts, fangs, and trophies on every model
- Runes and Fenrisian glyphs (freehand or transfers)
- Red eye lenses

*View color swatches: Open file in VSCode → Ctrl+Shift+V*
<!-- END BRUSHMASTER SECTION -->

---

<!-- ARTISAN SECTION - Tacticus ignore -->
## Assembly Guide

### Build Priority

Build characters and key melee units first to get playable games fastest.

| Priority | Unit | Reason |
|----------|------|--------|
| 1 | Wolf Priest #1 (Primaris Chaplain) | HQ required for every game, leads Blood Claws |
| 2 | Wolf Priest #2 (Primaris Chaplain) | Second HQ, leads Headtakers |
| 3 | Thunderwolf Cavalry x3 | Signature unit, high visual impact, Stormlance centrepiece |
| 4 | Wolf Guard Terminators x5 (Brick 1) | Deep Strike threat, pairs with Logan |
| 5 | Wolf Guard Terminators x5 (Brick 2) | Second Deep Strike brick, pairs with Arjac |
| 6 | Logan Grimnar | Warlord, attach to Terminator Brick 1 |
| 7 | Arjac Rockfist | Terminator leader, melee hammer |
| 8 | Blood Claws x10 | Melee blob, Wolf Priest attached |
| 9 | Grey Hunters x10 | Objective holders, Njal attached |
| 10 | Bjorn the Fell-Handed | CP generation, fire support |
| 11 | Njal Stormcaller | Psyker leader for Grey Hunters |
| 12 | Ballistus Dreadnought | Ranged fire support |
| 13 | Gladiator Reaper | Anti-horde fire support |
| 14 | Wolf Guard Headtakers | Character hunters |
| 15 | Wolf Scouts x5 | Infiltrate screening |

### Magnetization Recommendations

| Unit | What to Magnetize | Difficulty | Notes |
|------|-------------------|------------|-------|
| **Thunderwolf Cavalry** | Rider weapon arms (thunder hammer, wolf claw, storm shield, chainfist) | Medium | 3mm x 1mm magnets in shoulder sockets. Each rider has multiple weapon options -- magnetize all three riders to swap loadouts between games. |
| **Wolf Guard Terminators** | Arms (storm bolter, assault cannon, cyclone missile launcher, thunder hammer, storm shield, wolf claw, power fist) | Medium | 3mm x 1mm in shoulder joints. Lets you swap between shooty and melee loadouts per brick. Highly recommended -- the kit comes with all options. |
| **Long Fangs / Ballistus** | N/A (Ballistus is fixed loadout) | N/A | Ballistus has no weapon options. If you swap Long Fangs in later, magnetize heavy weapons (lascannon, heavy bolter, multi-melta, plasma cannon). |
| **Gladiator Reaper** | Hull weapon (Reaper vs Lancer vs Valiant) | Hard | If you want to run different Gladiator variants from one kit, magnetize the turret and hull weapons. Large magnets (5mm x 2mm) required. |
| **Wolf Priests** | N/A | N/A | Fixed loadout characters -- no magnetization needed. |

> **Tip:** At minimum, magnetize the Thunderwolf Cavalry riders and Terminator arms. These two kits have the most loadout flexibility and you will want to adjust between tournaments.

### Shopping List

| Product | Qty | Approx USD | Notes |
|---------|-----|------------|-------|
| Logan Grimnar | 1 | $42 | Epic Hero (foot version) |
| Arjac Rockfist | 1 | $35 | Epic Hero |
| Bjorn the Fell-Handed | 1 | $55 | Epic Hero dreadnought |
| Njal Stormcaller | 1 | $42 | Epic Hero psyker |
| Primaris Chaplain | 2 | $70 | Build as Wolf Priest x2 |
| Space Wolves Upgrade Pack | 1 | $15 | Wolf bits for Chaplain conversions |
| Wolf Guard Terminators | 2 | $120 | 5-model boxes, one per brick |
| Wolf Guard Headtakers | 1 | $55 | 3 models + 3 Hunting Wolves |
| Space Wolves Pack | 2 | $110 | 1x Grey Hunters, 1x Blood Claws |
| Thunderwolf Cavalry | 1 | $70 | 3-model box |
| Wolf Scouts | 1 | $35 | 5-model box |
| Assault Intercessors | 1 | $55 | For 500pt Colosseum list |
| Gladiator Reaper | 1 | $70 | Tank kit |
| Ballistus Dreadnought | 1 | $70 | Dreadnought |
| Magnetization supplies | 1 | $15 | 3mm x 1mm rare earth magnets (50-pack), pin vise, super glue |
| **TOTAL** | | **~$859** | Full 2000pt army + magnets |
<!-- END ARTISAN SECTION -->

---

*Built by Tacticus for competitive play. Vlka Fenryka!*

<!-- CHRONICLER SECTION - Tacticus ignore -->
## Campaign Metadata

- **campaign_ready:** false
- **battles_played:** 0
- **win_record:** 0-0-0
- **crusade_compatible:** true
- **narrative_hooks:** Derived from army lore section above
<!-- END CHRONICLER SECTION -->
